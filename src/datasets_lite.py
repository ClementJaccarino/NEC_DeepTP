...
class DatasetEncoderDecoder:
    ...
    def _calc_latlon_spd(self, track_dataframe):
        ...
        CenterTraj.reset_index(drop=True, inplace=True)
        # CenterTraj.loc[:, 'azimuth'] = last_pnt
        tmp_df = CenterTraj.shift(-1)
        latlon_spd = np.divide((tmp_df[['Lat', 'Lon']].values - CenterTraj[['Lat', 'Lon']].values),
                               tmp_df.DT.values.reshape(-1, 1), where=tmp_df.DT.values.reshape(-1, 1)!=0)
    ...
    def next_batch(self):
        ...
        # print("Self train feature : ")
        # print(self.train_feature_cubes)
        # print("Type : ")
        # print(self.train_feature_cubes.shape)
